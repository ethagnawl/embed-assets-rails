...
module Saulabs::EmbedAssets
  ...
  class Processor < Tilt::Template
    ...
    def embeddable?(asset_path)
      ...
      puts "EMBED_ASSETS_RAILS" + asset_path.to_s.match(EMBEDDABLE) && asset_path.exist?
      ...
      puts "EMBED_ASSETS_RAILS" + EMBED_EXTS.include?(asset_path.extname)      
      return false unless font || encoded_contents(asset_path).length < MAX_IMAGE_SIZE
      puts "EMBED_ASSETS_RAILS" + (font || encoded_contents(asset_path).length < MAX_IMAGE_SIZE)
      return true
    end
